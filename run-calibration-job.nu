...
# validation
# $populations = (parse_string_list_ints $populations)
# $access_rates = (parse_string_list_floats $access_rates)
# $betas = (parse_string_list_floats $betas)
...
def parse_string_list_floats [str: string] {
    try {
        return ($str | split row "," | into float )
    } catch {
        |err| $err.msg;
    }
    try {
